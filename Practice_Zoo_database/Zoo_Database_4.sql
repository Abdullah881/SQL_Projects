...
/*
Get total cost of nutrition 
*/

DECLARE @totalHab MONEY;
DECLARE @totalNut MONEY;
DECLARE @results2 MONEY;

SET @totalHab = (SELECT SUM(CONVERT(int,habitat_cost)) FROM tbl_habitat);
SET @totalNut = (SELECT SUM(nutrition_cost) FROM tbl_nutrition);
SET @results2 = (@totalHab + @totalNut);

Print (
    CONVERT(VARCHAR(50),@totalHab) + CHAR(9) + '- The Total Habitat Cost' + CHAR(13) + 
    CONVERT(VARCHAR(50),@totalNut) + CHAR(9) + '- The Total Nutrition Cost' + CHAR(13)
